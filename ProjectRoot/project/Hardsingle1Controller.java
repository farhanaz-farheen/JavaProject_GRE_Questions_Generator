 ...
 * @author User
 */
public class Hardsingle1Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        String message = "HardSingleBlank\n" + nam;
        try{
        Socket sock = new Socket(InetAddress.getLocalHost(),10893);
            ...
            if(warning.contains("6taken")){
                //System.out.println("WHAT IS THIS");
                f1=1;
                Accessinguser.getcheck().setcode("6istaken");
                ...
                outgoing.writeUTF("6istaken");
                DataInputStream input = new DataInputStream(socket2.getInputStream());
                incoming = input.readUTF();
                socket2.close();
            }
            else if(warning.contains("6free")){
            Accessinguser.getcheck().setcode("6isfree"); 
            Accessinguser.getcheck().settime("ANSWERS:");
            Socket socket2 = new Socket(InetAddress.getLocalHost(),10893);
            DataOutputStream outgoing = new DataOutputStream(socket2.getOutputStream());
            outgoing.writeUTF("6isfree\n" + nam + "6");
            DataInputStream input = new DataInputStream(socket2.getInputStream());
            incoming = input.readUTF();
            socket2.close();
            }
