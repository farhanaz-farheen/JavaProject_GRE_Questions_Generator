 ...
 * @author User
 */
public class SingleblankwindowController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        String message = "EasySingleBlank\n" + nam;
        try{
        Socket sock = new Socket(InetAddress.getLocalHost(),10893);
            ...
            if(warning.contains("1taken")){
                //System.out.println("WHAT IS THIS");
                f1=1;
                Accessinguser.getcheck().setcode("1istaken");
                //Project.showsingleans1();
                ...
                outgoing.writeUTF("1istaken");
                DataInputStream input = new DataInputStream(socket2.getInputStream());
                incoming = input.readUTF();
                socket2.close();
            }
            else if(warning.contains("1free")){
            Accessinguser.getcheck().setcode("1isfree"); 
            Accessinguser.getcheck().settime("ANSWERS:");
            Socket socket2 = new Socket(InetAddress.getLocalHost(),10893);
            DataOutputStream outgoing = new DataOutputStream(socket2.getOutputStream());
            outgoing.writeUTF("1isfree\n" + nam + "1");
            DataInputStream input = new DataInputStream(socket2.getInputStream());
            incoming = input.readUTF();
            socket2.close();
            }
